...
- 狀態:Todo(2026-07-18 建立;raw 資料已解包完成)
- 優先級:高
- 對應需求:FR-1、FR-6
- 相依:無(raw_v1 已就緒)
- 描述:寫轉換腳本把 backend/data/source/raw_v1/(遊戲 1.0 解包)轉成匯入格式,取代 2024-01 舊資料;同時修正「無技能即排除」誤殺純輔助帕魯的規則(趴趴鯰案例);評估 partner_skill schema 擴充以表達新版結構化夥伴技能(各專注階數值、StackBuff 等新效果),並讓真實加成值取代 +10% 佔位值
- 驗收條件:
  - [ ] 全部新帕魯(753 列中符合 Q-D1 範圍者)匯入,繁中名稱正確
  - [ ] 波魯傑克斯等改版輔助的夥伴技能正確參與推薦計算
