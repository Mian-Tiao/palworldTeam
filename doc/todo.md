...
- 驗收條件:
  - [ ] 至少評估 2 個候選資料源並列出比較
  - [ ] 使用者已確認採用的資料源(Q-3 結案)
  - [ ] 變種帕魯識別方式(Q-D1)與夥伴技能效果格式(Q-D2)隨資料源格式一併定案
...
- 驗收條件:
  - [ ] 公式版本經使用者確認(Q-6 結案),含等級換算方式
  - [ ] 頭目防禦/等級是否參與計算定案(Q-D3 結案)
...
| 編號 | 項目 | 待確認事項 | 來源 | 解鎖方式 |
|------|------|-----------|------|----------|
| P-1 | 正式資料匯入腳本(全量替換+import_log) | Q-3、Q-D1、Q-D2(資料源與格式) | requirements / data-model | T-2 完成並經使用者確認 |
| P-2 | 傷害計算器(services/,含單元測試) | Q-6、Q-D3(公式版本) | requirements / architecture | T-3 完成並經使用者確認 |
| P-3 | 推薦引擎與 `POST /api/team-recommendations` | Q-1(隊伍上限)+ 依賴 P-2 | requirements | 使用者確認 Q-1;P-2 完成 |
| P-4 | 頭目清單資料與頭目選擇 UI | Q-2(初版收錄範圍) | requirements / architecture | 使用者確認收錄清單 |
