...
| 疊層型夥伴技能(P-8) | 1.0 版新增(如波魯傑克斯命中疊攻、焰煌擊殺疊攻):每層固定 %,實戰層數隨命中/擊殺變動且會衰退,無法精確模擬。統一以「實際上限層數 × 維持係數 0.5(取半)」估期望層數(至少 1 層),換算成 flat 加成納入。上限自遊戲描述「最多可累積 N 層」解析;未知時退回 10。係數為工程近似,非遊戲數值 |
| 隨機浮動 | ±10% 取期望值,不納入 |
